...
class TransformerLensSAE(nn.Module):
    def __init__(self, layer_index=6, location="resid_post_mlp"):
        ...
        #     "/data/models/gpt2-local",
        #     local_files_only=True
        # )

        base_model = transformer_lens.HookedTransformer.from_pretrained_no_processing(
            #config=hf_config,
            model_name="gpt2",  # 模型结构名称，用于设定 config
            # model_name="local-gpt2",# dummy name to skip error
            # model_path="/data/models/gpt2-local",  # 你的本地目录
